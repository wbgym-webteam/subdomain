...
def get_selections_from_db(db):
    try:
        ...
        return db.session.execute(
            text(
                "SELECT Student_id AS student_id, Course_id AS course_id, weight AS weight FROM pt_wishes"
            )
...
def get_students_from_workbook(file_path=None):
    try:
        ...
        if file_path is None:
            ...
            file_path = os.path.join(current_dir, 'data', 'pt', 'students.xlsx')
...
def SelectionExporter(db, file_path=None):
    try:
        ...
        if file_path is None:
            current_dir = os.path.dirname(os.path.abspath(__file__))
            file_path = os.path.join(current_dir, 'data', 'pt', 'downloads', 'Kurs_Wuensche.xlsx')
        ...
        if selections:
            for selection in selections:
                ...
                if "user_id" in selection and "presentation_id" in selection and "weight" in selection:
                    sheet.append([selection["user_id"], selection["presentation_id"], selection["weight"]])
                else:
